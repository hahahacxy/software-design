...
if (isset($_POST["login"])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM admin WHERE admin_name = ? AND password_admin = ?");
    ...
    if ($user_result->num_rows == 1) {
        ...
        // 插入 online 表
        $insert_stmt = $conn->prepare("INSERT INTO online (login_time, kind, login_name, login_ip) VALUES (NOW(), 'admin', ?, ?)");
        ...
        // 跳转到主页
        header("Location: http://172.20.10.4/index_admin.php");
        exit;
    } else {
        echo "<script>
            alert('用户名或密码无效!');
            window.location.href = 'http://172.20.10.4/login_admin.php';
        </script>";
    }
